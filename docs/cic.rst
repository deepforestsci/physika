...
Every inductive type have an elimination rule known as a recursor (for ``Nat``, there is a ``Nat.rec``).
A recursor, as CIC ``Expr`` terms, contains principles of induction used for both proving theorems about
the type and to define recursive functions to compute with.
