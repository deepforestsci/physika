...
print(dft_spectrum)
print(torch.abs(dft_spectrum if isinstance(dft_spectrum, torch.Tensor) else torch.tensor(float(dft_spectrum))))
print(idft(dft_spectrum))
fft_spectrum = ct_fft(signal, 4)
print(fft_spectrum)
print(torch.abs(fft_spectrum if isinstance(fft_spectrum, torch.Tensor) else torch.tensor(float(fft_spectrum))))
print(ict_fft(fft_spectrum, 4))
print(dft(signal))
print(ct_fft(signal, 4))
print(torch.fft.fft(signal))
print(idft(dft_spectrum))
print(ict_fft(fft_spectrum, 4))
print(torch.fft.ifft(fft_spectrum))
Ns = 9
center = 4.0
...
print(torch.abs(spectrum_1d if isinstance(spectrum_1d, torch.Tensor) else torch.tensor(float(spectrum_1d))))
print(torch.fft.ifft(spectrum_1d))
matrix = torch.stack([torch.as_tensor(torch.stack([torch.as_tensor(1), torch.as_tensor(2), torch.as_tensor(3)])), torch.as_tensor(torch.stack([torch.as_tensor(4), torch.as_tensor(5), torch.as_tensor(6)])), torch.as_tensor(torch.stack([torch.as_tensor(7), torch.as_tensor(8), torch.as_tensor(9)]))])
spectrum_2d = torch.fft.fft2(matrix)
print(spectrum_2d)
print(torch.fft.ifft2(spectrum_2d))
tensor = torch.stack([torch.as_tensor(torch.stack([torch.as_tensor(torch.stack([torch.as_tensor(1), torch.as_tensor(2)])), torch.as_tensor(torch.stack([torch.as_tensor(3), torch.as_tensor(4)]))])), torch.as_tensor(torch.stack([torch.as_tensor(torch.stack([torch.as_tensor(5), torch.as_tensor(6)])), torch.as_tensor(torch.stack([torch.as_tensor(7), torch.as_tensor(8)]))]))])
spectrum_Nd = torch.fft.fftn(tensor)
print(spectrum_Nd)
print(torch.fft.ifftn(spectrum_Nd))
x = torch.as_tensor(torch.stack([torch.as_tensor(1.0), torch.as_tensor(2.0), torch.as_tensor(3.0), torch.as_tensor(4.0)])).requires_grad_(True).to(DEVICE)
X = torch.fft.fft(x)
print(X)
y = torch.real(X[int(1)] if isinstance(X[int(1)], torch.Tensor) else torch.tensor(float(X[int(1)])))
print(y)
print(compute_grad(y, x))
dft_spectrum = dft(x)
...
print(compute_grad(dft_coeff, x))
...
print(compute_grad(ct_fft_coeff, x))
...
print(inv_sample)
print(compute_grad(inv_sample, x))
x2 = torch.as_tensor(torch.stack([torch.as_tensor(1.0), torch.as_tensor(2.0), torch.as_tensor(3.0), torch.as_tensor(4.0), torch.as_tensor(5.0), torch.as_tensor(6.0), torch.as_tensor(7.0), torch.as_tensor(8.0)])).requires_grad_(True).to(DEVICE)
X2 = torch.fft.fft(x2)
...
print(energy_time)
print(energy_freq)
print(compute_grad(energy_time, x2))
print(compute_grad(energy_freq, x2))