...
def ct_fft(x, s):
    if s == 1.0:
        return x
    half_size = (s / 2)
    even = torch.stack([x[int((2 * a))] for _fi_a in range(int(half_size)) for a in [torch.tensor(float(_fi_a))]])
    odd = torch.stack([x[int(((2 * a) + 1))] for _fi_a in range(int(half_size)) for a in [torch.tensor(float(_fi_a))]])
    E = ct_fft(even, half_size)
    O = ct_fft(odd, half_size)
    twiddle = torch.stack([(torch.cos((((2 * π) * k) / s) if isinstance((((2 * π) * k) / s), torch.Tensor) else torch.tensor(float((((2 * π) * k) / s)))) - (1j * torch.sin((((2 * π) * k) / s) if isinstance((((2 * π) * k) / s), torch.Tensor) else torch.tensor(float((((2 * π) * k) / s)))))) for _fi_k in range(int(half_size)) for k in [torch.tensor(float(_fi_k))]])
    lower = torch.stack([(E[int(k)] + (twiddle[int(k)] * O[int(k)])) for _fi_k in range(int(half_size)) for k in [torch.tensor(float(_fi_k))]])
    upper = torch.stack([(E[int(k)] - (twiddle[int(k)] * O[int(k)])) for _fi_k in range(int(half_size)) for k in [torch.tensor(float(_fi_k))]])
