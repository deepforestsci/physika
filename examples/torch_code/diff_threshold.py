...
def L(t):
    ...
    return torch.where(t > 0.5, torch.as_tensor(((3.0 * ((t - 0.75) ** 2.0)) + 0.1)).float(), torch.as_tensor(((t ** 2.0) + 2.0)).float())
...
physika_print(L(t3))
physika_print(compute_grad(L, t3))
physika_print(t3)
s0 = 0.3
physika_print(L(s0))
...
physika_print(L(s2))
physika_print(compute_grad(L, s2))
s3 = (s2 - (0.2 * compute_grad(L, s2)))
physika_print(L(s3))
physika_print(compute_grad(L, s3))
s4 = (s3 - (0.2 * compute_grad(L, s3)))
physika_print(L(s4))
physika_print(compute_grad(L, s4))
physika_print(s4)