...
physika_print(c.x)
physika_print(c.y)
pos0 = torch.tensor([0.0, 10.0], device='cpu')
vel0 = torch.tensor([1.0, 0.0], device='cpu')
gravity = torch.tensor([0.0, (-9.81)], device='cpu')
...
v = torch.as_tensor(torch.tensor([2.0, 3.4], device='cpu')).requires_grad_(True).to('cpu')
ke0_v = ke_wrt_vel(v)
physika_print(ke0_v)
