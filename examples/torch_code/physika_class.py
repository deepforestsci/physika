...
print(c.x)
print(c.y)
pos0 = torch.stack([torch.as_tensor(0.0), torch.as_tensor(10.0)])
vel0 = torch.stack([torch.as_tensor(1.0), torch.as_tensor(0.0)])
gravity = torch.stack([torch.as_tensor(0.0), torch.as_tensor((-9.81))])
...
print(p2.pos)
v = torch.as_tensor(torch.stack([torch.as_tensor(2.0), torch.as_tensor(3.4)])).requires_grad_(True).to(DEVICE)
ke0_v = ke_wrt_vel(v)
print(ke0_v)
