from physika.features.randomness import (extract_dist_args, sample, normal_dist, uniform_dist, beta_dist, gamma_dist, bernoulli_dist)
...
class TestRandomessHelpers:
    ...
    def test_sample(self):
        ...
        estimator = "reparam"
        mean, std = "0.0", "1.0"
        result = sample("torch.distributions.Normal(%s, %s)"%(mean, std), [], mode=estimator, default_mode=estimator, to_expr=to_expr)
        assert result == 'torch.distributions.Normal(%s, %s).rsample()'%(mean, std)

        # estimator node is none, but default reparam used
        result = sample("torch.distributions.Normal(%s, %s)"%(mean, std), [], mode="none", default_mode=estimator, to_expr=to_expr)
        assert result == 'torch.distributions.Normal(%s, %s).rsample()'%(mean, std)

        # sample a tensor with shape ℝ[20, 1]
        shape_args = [20, 1]
        result = sample("torch.distributions.Normal(%s, %s)"%(mean, std), shape_args=shape_args, mode="none", default_mode=estimator, to_expr=to_expr)
        assert result == 'torch.distributions.Normal(%s, %s).rsample((int(%s), int(%s),))'%(mean, std, shape_args[0], shape_args[1])

        # sample a tensor with shape ℝ[20, 1, 3] and mean/std 3, 0.5
        shape_args = [20, 1, 3]
        mean_d, std_d = "3", "0.5"
        result = sample("torch.distributions.Normal(%s, %s)"%(mean_d, std_d), shape_args=shape_args, mode="none", default_mode=estimator, to_expr=to_expr)
        assert result == 'torch.distributions.Normal(%s, %s).rsample((int(%s), int(%s), int(%s),))'%(mean_d, std_d, shape_args[0], shape_args[1], shape_args[2])

        # none estimator produces .sample()
        result = sample("torch.distributions.Normal(%s, %s)"%(mean, std), [], mode="none", default_mode="none", to_expr=to_expr)
        assert result == 'torch.distributions.Normal(%s, %s).sample()'%(mean, std)

        # Bernoulli distribution with score function estimator
        p = "0.5"
        result = sample("torch.distributions.Bernoulli(%s)"%(p), [], mode="score", default_mode="none", to_expr=to_expr)
        assert result == 'torch.distributions.Bernoulli(%s).sample().detach()'%(p)
...
class TestProbDistributionsCodegen:
    ...
    def test_normal_dist(self):
        ...
        assert result == "torch.distributions.Normal(0.0, 1.0).rsample((int(20.0),))"

        # vector sample ℝ[20, 3, 3]
        n0, n1, n2 = ("num", 20.0), ("num", 3.0), ("num", 3.0)
        result = normal_dist([mu, sigma, n0, n1, n2], to_expr)
        assert result == "torch.distributions.Normal(0.0, 1.0).rsample((int(20.0), int(3.0), int(3.0),))"

        # grad mode ooverride
        result = normal_dist([mu, sigma, ("string", "score")], to_expr)
        assert result == "torch.distributions.Normal(0.0, 1.0).sample().detach()"
    ...
    def test_uniform_dist(self):
        ...
        assert result == "torch.distributions.Uniform(0.0, 1.0).rsample((int(10.0),))"

        # vector sample ℝ[20, 3, 3]
        n0, n1, n2 = ("num", 20.0), ("num", 3.0), ("num", 3.0)
        result = uniform_dist([lo, hi, n0, n1, n2], to_expr)
        assert result == "torch.distributions.Uniform(0.0, 1.0).rsample((int(20.0), int(3.0), int(3.0),))"

        # explicit score override
        result = uniform_dist([lo, hi, ("string", "score")], to_expr)
        assert result == "torch.distributions.Uniform(0.0, 1.0).sample().detach()"
    ...
    def test_beta_dist(self):
        ...
        assert result == "torch.distributions.Beta(0.5, 0.5).rsample((int(8.0),))"

        # vector sample ℝ[20, 3, 3]
        n0, n1, n2 = ("num", 20.0), ("num", 3.0), ("num", 3.0)
        result = beta_dist([alpha, beta, n0, n1, n2], to_expr)
        assert result == "torch.distributions.Beta(0.5, 0.5).rsample((int(20.0), int(3.0), int(3.0),))"

        # explicit score override
        result = beta_dist([alpha, beta, ("string", "score")], to_expr)
        assert result == "torch.distributions.Beta(0.5, 0.5).sample().detach()"
    ...
    def test_gamma_dist(self):
        ...
        assert result == "torch.distributions.Gamma(1.0, 1.0).rsample((int(5.0),))"

        # vector sample ℝ[20, 3, 3]
        n0, n1, n2 = ("num", 20.0), ("num", 3.0), ("num", 3.0)
        result = gamma_dist([concentration, rate, n0, n1, n2], to_expr)
        assert result == "torch.distributions.Gamma(1.0, 1.0).rsample((int(20.0), int(3.0), int(3.0),))"

        # explicit score override
        result = gamma_dist([concentration, rate, ("string", "score")], to_expr)
        assert result == "torch.distributions.Gamma(1.0, 1.0).sample().detach()"
    ...
    def test_bernoulli_dist(self):
        ...
        assert result == "torch.distributions.Bernoulli(0.5).sample((int(20.0),)).detach()"

        # vector sample ℝ[20, 3, 3]
        n0, n1, n2 = ("num", 20.0), ("num", 3.0), ("num", 3.0)
        result = bernoulli_dist([p, n0, n1, n2], to_expr)
        assert result == "torch.distributions.Bernoulli(0.5).sample((int(20.0), int(3.0), int(3.0),)).detach()"
