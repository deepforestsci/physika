...
class TestChachiyoCorrelation:
    ...
    def test_matches_reference(self, xc_ns):
        ...
        vc_expected = ec_expected + a * b * (2.0 + rs) / (
            3.0 * (b + b * rs + rs ** 2.0))
...
class TestVWNCorrelation:

    def test_matches_reference(self, xc_ns):
        rs = ref_wigner_seitz_radius(N_TEST)
        A, b, c, x0 = 0.0310907, 3.72744, 12.9352, -0.10498
        x = torch.sqrt(rs)
        X = rs + b * x + c
        Q = (4.0 * c - b ** 2.0) ** 0.5
        ...
        ec_expected = A * (torch.log(rs / X) + 2.0 * b / Q * tanx -
                           fx0 * (torch.log((x - x0) ** 2.0 / X) +
                                  f3 * tanx))
        tt = tx ** 2.0 + Q ** 2.0
        vc_expected = ec_expected - x * A / 6.0 * (
            2.0 / x - tx / X - 4.0 * b / tt - fx0 *
            (2.0 / (x - x0) - tx / X - 4.0 * (2.0 * x0 + b) / tt))

        out = xc_ns["lda_c_vwn"](N_TEST)
        assert torch.allclose(out[0], ec_expected, rtol=r_tol, atol=a_tol)
        assert torch.allclose(out[1], vc_expected, rtol=r_tol, atol=a_tol)

    def test_energy_density_is_negative(self, xc_ns):
        ec, _ = xc_ns["lda_c_vwn"](N_TEST)
        assert (ec.real < 0).all()

    def test_close_to_chachiyo(self, xc_ns):
        # Two different fits of the same correlation functional should
        # agree to within a few percent over this density range.
        ec_vwn, _ = xc_ns["lda_c_vwn"](N_TEST)
        ec_chachiyo, _ = xc_ns["lda_c_chachiyo"](N_TEST)
        assert torch.allclose(ec_vwn.real, ec_chachiyo.real, rtol=0.05,
                              atol=0.005)
