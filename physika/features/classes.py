...
def build_class(constructor_params: Optional[list],
    ...
    []
    >>> result
    {'constructor_params': [('mass', 'ℝ')], 'fields': [], 'methods': [{'name': 'ke', 'params': [], 'return_type': 'ℝ', 'statements': [], 'body': None}]}
    """
    ...
    if constructor_params is None:
        ...
        return {"constructor_params": fields, "fields": [],
                "methods": methods}
...
def emit_method(method: dict, all_params: list, to_expr: Callable,
    ...
    method: dict
        ...
        "params" with declared types, "return_type", body statements and expressions.
    all_params: list
        List of method's parameters with declared types.
    to_expr: Callable
        ``ast_to_torch_expr`` for getting the associated torch code.
    scalar_only: bool
        Boolean that indicate how to define if-else blocks when emiting body stmts.
    ...
    for pname, ptype in params:
        if is_learnable(ptype):
            method_lines.append(f"        {pname} = torch.as_tensor({pname}).float()")
...
def generate_class(name: str, class_def: dict) -> list[str]:
    ...
    >>> class_def = build_class(None, [("field_decl", "mass", "ℝ"), ("method_def", ke)])
    ...
    for pname, ptype in constructor_params:
        ...
        if isinstance(ptype, tuple) and ptype[0] == "struct_type":
            ...
            if has_forward:
                # adds nn.Parameter
                class_lines.append(
                    f"        self.{pname} = nn.Parameter(torch.as_tensor({pname}).float())"
                )
            else:
            ...
            class_lines.append(
                f"        self.{pname} = torch.as_tensor({pname}).float() "
                f"if isinstance({pname}, (int, float, torch.Tensor)) else {pname}"
            )
    ...
    for method in methods:
        ...
        class_lines.extend(
            emit_method({**method, "params": method_params},
...
def make_parser_rules():
    ...
    def p_class_method_params_simple(p):
        ...
        # Method with params and a single return expression (no statements between return and method definition).
        # Example:
        #   def dot(other: Vec) → ℝ:
