...
def plot_ising_result(spins_before: Any, spins_after: Any,
                      J: Any, h: Any) -> None:
    """Plot before/after spin configurations for a 1-D Ising chain.

    Displays a 1×2 bar-chart comparison of spin states before and after
    optimisation, annotated with energy and magnetisation.

    Parameters
    ----------
    spins_before : torch.Tensor
        Spin vector (values +-1) sampled before optimisation.
    spins_after : torch.Tensor
        Spin vector (values +-1) sampled after optimisation.
    J : float
    ...
    def energy(spins) -> float:
        return float(
            -J * torch.sum(spins[:-1] * spins[1:]) - h * torch.sum(spins))

    def mag(spins) -> float:
        return float(torch.mean(spins))

    def colors(spins) -> list:
        return ['#2196F3' if s > 0 else '#F44336' for s in spins.tolist()]

    n = len(spins_before)
    e_before = energy(spins_before)
    e_after = energy(spins_after)
    m_before = mag(spins_before)
    m_after = mag(spins_after)
    ...
    ax0.set_title(f'Before  E={e_before:.1f},  M={m_before:+.2f}')
    ...
    ax1.set_title(f'After   E={e_after:.1f},  M={m_after:+.2f}')
